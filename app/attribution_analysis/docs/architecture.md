...
- `calculate_portfolio_value()` 负责生成底层估值序列，非交易日也会继续按持仓估值
- `build_performance_curves()` 从同一份估值序列派生两条曲线：
  - 账户净值曲线：包含外部资金流
  - 策略收益曲线：剔除外部资金流，用于 TWR / Alpha / Beta / Sharpe
...
| `attribution.py` | 双口径归因分析（纯 CSV 消费） | CSV 数据文件 | 分析报告 (.md) |
...
- **双口径输出**：账户净值口径回答“账户实际体验”，策略收益口径回答“交易决策是否有效”
